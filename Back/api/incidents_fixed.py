...
def report_incident():
    ...
    try:
        ...
        # ✅ FIX: 타임아웃 추가 - 5초 내에 락 획득 못하면 실패
        if not incident_lock.acquire(timeout=5):
            print(f"⚠️ 다른 사고 처리 중 - 타임아웃")
            return jsonify({'error': 'Server busy processing another incident, try again'}), 503
