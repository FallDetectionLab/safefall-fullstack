...
def get_dashboard_stats():
    ...
    try:
        ...
        stats_query = db.session.query(
            ...
            func.sum(db.case((Incident.is_checked == True, 1), else_=0)).label('checked'),
            func.sum(db.case((Incident.detected_at >= today_start, 1), else_=0)).label('today')
...
def get_recent_videos():
    ...
    try:
        ...
        # 최근 영상 조회
        incidents = Incident.query.filter_by(user_id=current_user_id)\
...
def get_incidents_summary():
    ...
    try:
        ...
        return jsonify({
            ...
            'by_type': by_type,
            'by_date': by_date,
            'incidents': [incident.to_dict() for incident in incidents[:10]]  # 최근 10개
        }), 200
