...
function LoginPage({ 
  ...
  const handleAutoLogin = async (username, password, serviceName) => {
    try {
      ...
      if (error.response) {
        ...
        if (status === 401) {
          setError(`${serviceName} 로그인 실패: 아이디 또는 비밀번호가 올바르지 않습니다.`);
        } else if (status === 403) {
          setError(`${serviceName} 로그인 실패: 계정이 비활성화되었습니다. 관리자에게 문의하세요.`);
        } else if (status === 500) {
          setError(`${serviceName} 로그인 실패: 서버 오류가 발생했습니다. 잠시 후 다시 시도해주세요.`);
        } else {
          setError(`${serviceName} 로그인 중 오류가 발생했습니다. (오류 코드: ${status})`);
        }
      } else if (error.request) {
        setError(`${serviceName} 로그인 실패: 서버에 연결할 수 없습니다. 네트워크 연결을 확인해주세요.`);
      } else {
  ...
  // 각 서비스별 로그인 핸들러
  const handleNaverLogin = () => handleAutoLogin("naver", "12345678!", "Naver");
  const handleGoogleLogin = () => handleAutoLogin("google", "12345678!", "Google");
  const handleKakaoLogin = () => handleAutoLogin("kakao", "12345678!", "Kakao");
